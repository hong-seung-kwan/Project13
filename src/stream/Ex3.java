...
public class Ex3 {
	...
	public static void main(String[] args) {
		...
		// distinct()은 중복을 제거하고 새로운 자료를 생성
		// forEach()은 요소를 하나씩 차례대로 n에 대입하고, n을 사용하여 출력
		...
		// Stream<Integer>를 기본형 스트림 IntStream으로 변경
		...
		// map: 값을 다른 값으로 변경. 대문자로 변경
		List<String> strList = Arrays.asList("aaa", "bbb", "ccc");
		
		System.out.println();
		strList.stream().map(s -> s.toUpperCase()).forEach(s -> System.out.println(s + " "));
		
		// map: 값을 다른값으로 변경. 문자의 길이로 변경
		System.out.println();
		strList.stream().map(s -> s.length()).forEach(s -> System.out.println(s + " "));
